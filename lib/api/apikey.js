...
export async function handleRemoveApiKey(req, res, url) {
  ...
  // Kalau provider kehilangan SEMUA key (dan bukan OAuth), model-nya dianggap invalid
  const hasOAuth = provider.oauthType || provider.status === 'ready' && (provider.apiKeys || []).length === 0;
  if (provider.apiKeys.length === 0 && !hasOAuth) {
    ...
    if (combosChanged > 0) {
      log.info('combos pruned after last key removed', {
