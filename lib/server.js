...
import { handleTestAllModels } from './api/test-all.js';
...
import { renderTokenSaver } from './ui/token-saver.js';
import {
  handleTokenSaverStatus,
  handleTokenSaverInstall,
  handleTokenSaverConfigGet,
  handleTokenSaverConfigSet
} from './api/token-saver.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const ICONS_DIR = join(__dirname, 'ui', 'icons');
...
function isLocalRequest(req) {
  ...
  const isLocalSocket = ra === '127.0.0.1' || ra === '::1' || ra === '::ffff:127.0.0.1';
...
function errorType(status) {
  if (status === 400 || status === 401) return 'invalid_request_error';
...
export function startServer() {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      const isOAuthCallback = url.pathname === '/oauth/callback' || url.pathname.startsWith('/api/oauth/');

      if (isApi && !isOAuthCallback && !isLocalRequest(req)) {
        try {
          requireClient(req);
        } catch {
          return json(res, 401, {
      ...
      if (url.pathname === '/token-saver') return html(res, 200, renderTokenSaver());
      ...
      if (url.pathname === '/api/providers/remove-model' && req.method === 'POST') return await handleRemoveModel(req, res, url);
      if (url.pathname === '/api/test-model')          return await handleTestModel(req, res, url);
      if (url.pathname === '/api/test-all-models')     return await handleTestAllModels(req, res, url);
      ...
      if (url.pathname === '/api/update/state')  {
        ...
        return res.end(JSON.stringify(getUpdateState()));
      }

      // ===== Token Saver =====
      if (url.pathname === '/api/token-saver/status')  return await handleTokenSaverStatus(req, res);
      if (url.pathname === '/api/token-saver/install') return await handleTokenSaverInstall(req, res);
      if (url.pathname === '/api/token-saver/config' && req.method === 'GET')  return await handleTokenSaverConfigGet(req, res);
      if (url.pathname === '/api/token-saver/config' && req.method === 'POST') return await handleTokenSaverConfigSet(req, res);
      ...
      if (url.pathname.startsWith('/icons/')) {
        ...
        if (file.startsWith(ICONS_DIR) && existsSync(file)) {
          res.writeHead(200, {
            'Content-Type': iconContentType(name),
  ...
  server.on('error', (err) => {
    ...
    if (err.code === 'EADDRINUSE') {
      console.error('[server] Port ' + cfg.port + ' sudah dipakai. Set port lain di loka.json');
    }
  });
