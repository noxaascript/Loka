...
import { handleTunnelSetup, handleTunnelStart, handleTunnelStop, handleTunnelStatus, handleTunnelUninstall } from './api/tunnel.js';
...
export function startServer() {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      // ===== OAuth callback (public, no auth) — HARUS sebelum route lain =====
      ...
      // ===== Provider management =====
      if (url.pathname === '/api/test-provider')       return handleTestProvider(req, res, url);
      if (url.pathname === '/api/test-model')          return handleTestModel(req, res, url);
      if (url.pathname === '/api/tunnel/setup')        return handleTunnelSetup(req, res, url);
      if (url.pathname === '/api/tunnel/start')        return handleTunnelStart(req, res, url);
      ...
      if (url.pathname === '/api/cli/configure')       return handleCLIConfigure(req, res, url);
      if (url.pathname === '/api/cli/reset')           return handleCLIReset(req, res, url);
      if (url.pathname === '/api/add-api-key')         return handleAddApiKey(req, res, url);
      if (url.pathname === '/api/remove-api-key')      return handleRemoveApiKey(req, res, url);
      if (url.pathname === '/api/import-models')       return handleImportModels(req, res, url);
      if (url.pathname === '/api/connect-provider')    return handleConnectProvider(req, res, url);
      if (url.pathname === '/api/disconnect-provider') return handleDisconnectProvider(req, res, url);

      // ===== OAuth =====
      if (url.pathname === '/api/oauth/start')           return handleOAuthStart(req, res, url);
      if (url.pathname === '/api/oauth/poll')            return handleOAuthPoll(req, res, url);
      if (url.pathname === '/api/oauth/start-localhost') return handleOAuthStartLocalhost(req, res, url);
      if (url.pathname === '/api/oauth/poll-status')     return handleOAuthStatus(req, res, url);
      if (url.pathname === '/api/oauth/manual-start')    return handleOAuthManualStart(req, res, url);
      if (url.pathname === '/api/oauth/manual-complete') return handleOAuthManualComplete(req, res, url);
      if (url.pathname === '/api/oauth/anthropic')          return handleOAuthManualForAnthropic(req, res, url);

      // ===== Combos =====
      if (url.pathname.startsWith('/api/combos/')) return handleCombos(req, res, url);

      // ===== API Keys =====
      if (url.pathname.startsWith('/api/keys/')) return handleKeys(req, res, url);
