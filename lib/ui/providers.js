...
const LOGOS = {
  ...
  'openai-codex': CDN + '/codex.svg',
  anthropic: CDN + '/anthropic.svg',
  'anthropic-oauth': CDN + '/anthropic.svg',
  claude: CDN + '/claude.svg',
  gemini: CDN + '/gemini.svg',
  'google-gemini-cli': CDN + '/gemini.svg',
  groq: CDN + '/groq.svg',
  'github-copilot': CDN + '/githubcopilot.svg',
  xai: CDN + '/grok.svg',
  ollama: CDN + '/ollama.svg',
  openrouter: CDN + '/openrouter.svg',
  mistral: CDN + '/mistral.svg',
  deepseek: CDN + '/deepseek.svg',
  together: CDN + '/together.svg',
  fireworks: CDN + '/fireworks.svg',
  perplexity: CDN + '/perplexity.svg',
  cerebras: CDN + '/cerebras.svg',
  xkiro: CDN + '/xkiro.svg'
};
...
function logoUrl(p) { return LOGOS[p.id] || LOGOS[p.type] || null; }
...
function logoHtml(p, size) {
  ...
  const initial = (p.id || '?').charAt(0).toUpperCase();
  const fbStyle = 'background:' + color + ';width:' + sz + 'px;height:' + sz + 'px;border-radius:10px;color:#fff;font-weight:700;font-size:' + Math.round(sz * 0.55) + 'px;align-items:center;justify-content:center';
  if (url) {
    return '<img src="' + url + '" alt="' + p.id + '" style="width:' + sz + 'px;height:' + sz + 'px;object-fit:contain" onerror="this.style.display=&quot;none&quot;;this.nextElementSibling.style.display=&quot;flex&quot;"><div style="display:none;' + fbStyle + '">' + initial + '</div>';
  }
  return '<div style="display:flex;' + fbStyle + '">' + initial + '</div>';
}
...
export function renderProviders() {
  ...
  const html = '<p class="muted">Klik provider untuk lihat detail dan connect.</p>' +
    ...
    (regCards ? '<div class="pgrid">' + regCards + '</div>' : '<p class="dim">Belum ada provider API key.</p>') +
