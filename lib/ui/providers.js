...
const PROVIDER_LOGOS = {
  'openai': ['https://cdn.simpleicons.org/openai/10a37f', '/icons/openai.svg'],
  'openai-codex': ['https://cdn.simpleicons.org/openai/10a37f', '/icons/openai.svg'],
...
async function addApiKey(pid) {
  var k = window.dialog.prompt("Masukkan API key untuk " + pid + ":");
  if (!k || !k.trim()) return;
...
async function removeApiKey(pid, idx) {
  if (!window.dialog.confirm("Hapus API key ini?")) return;
  try {
